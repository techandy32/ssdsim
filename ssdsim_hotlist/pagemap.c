...
/*********************************************************************************************************************
 * 2022/4/28    
 * 在指定的channel chip die plane中尋找還未被寫過的block來當作Hot/Cold Block
 * 回傳值為block的編號 
 *********************************************************************************************************************/ 
...
Status move_page(struct ssd_info * ssd, struct local *location, unsigned int * transfer_size)
{
    ...
    Hotness = Hotness_LRU(lpn);

    if(Hotness == 1){
        #ifdef MPDEBUG
            printf("Hot\n");
        #endif
        if( Hot_Block[location->plane] == -1 || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[Hot_Block[location->plane]].free_page_num == 0)
        {
        ...
        ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
            ...
            printf("Cold\n");
        #endif
        if( Cold_Block[location->plane] == -1 || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[Cold_Block[location->plane]].free_page_num == 0)
        {
