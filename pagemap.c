...
Status move_page(struct ssd_info * ssd, struct local *location, unsigned int * transfer_size)
{
    ...
    /* Hot/Cold Block number */
    static unsigned int *Hot_Block = NULL, *Cold_Block = NULL;
	if(Hot_Block == NULL && Cold_Block == NULL){
    	Hot_Block = (unsigned int*)malloc(sizeof(int)*ssd->parameter->plane_die);
    	Cold_Block = (unsigned int*)malloc(sizeof(int)*ssd->parameter->plane_die);
	}
	...
	unsigned int Hotness = Hotness_LRU(lpn);
    if(Hotness == 1){
        #ifdef MPDEBUG
            printf("Hot\n");
        #endif
        if( *(Hot_Block+location->plane) == NULL || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[*(Hot_Block+location->plane)].free_page_num == 0)
        {
            *(Hot_Block+location->plane) = get_new_block_LRU(ssd, location -> channel, location -> chip, location -> die, location -> plane);
        }
        ...
        ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = *(Hot_Block+location->plane);
        ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
            ...
            //printf("Cold\n");
        #endif
        if( *(Cold_Block+location->plane) == NULL || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[*(Cold_Block+location->plane)].free_page_num == 0)
        {
            *(Cold_Block+location->plane) = get_new_block_LRU(ssd, location -> channel, location -> chip, location -> die, location -> plane);
        }
        old_active_block = ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block;
        ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = *(Cold_Block+location->plane);
        ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
    }
